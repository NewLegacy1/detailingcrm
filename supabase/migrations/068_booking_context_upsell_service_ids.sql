-- Include service_ids in booking context upsells so the booking page can filter add-ons by selected service.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      order by s.category nulls last, s.sort_order, s.name
    ),
    '[]'::jsonb
  )
  into v_services
...
comment on function public.get_public_booking_context is 'Returns public booking page context. Includes service_ids in upsells for per-service add-on filtering on the booking page.';
