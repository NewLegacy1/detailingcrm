-- Multi-location: anon-safe RPC to list active locations for an org by slug, with optional distance.
...
comment on function public.get_public_booking_locations(text, decimal, decimal) is 'Returns active locations for org by booking slug; optional lat/lng for distance_km and sort by nearest. Anon may execute.';

grant execute on function public.get_public_booking_locations(text, decimal, decimal) to anon;
grant execute on function public.get_public_booking_locations(text, decimal, decimal) to authenticated;
