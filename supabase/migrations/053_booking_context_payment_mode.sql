-- Expose booking payment mode in public context: only 'deposit' or 'card_on_file' when org is Pro and has it enabled; else 'none'.
...
begin
  ...
  into v_org_id, v_org_name, v_org_logo_url, v_tagline, v_map_lat, v_map_lng, v_show_prices,
    v_primary_color, v_accent_color, v_theme,
  ...
  -- Pro only: expose deposit or card_on_file; Starter or no plan => 'none'
...
comment on function public.get_public_booking_context is 'Returns public booking page context by booking_slug. Includes bookingPaymentMode (none | deposit | card_on_file) only when org is Pro and has it enabled.';
