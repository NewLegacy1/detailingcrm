-- Expose service_mode and shop_address in booking context; Pro only for shop/both.
...
declare
  ...
  v_website text;
begin
  select id, name, logo_url, booking_tagline, map_lat, map_lng, service_radius_km,
    coalesce(booking_show_prices, true),
    primary_color, accent_color, coalesce(theme, 'dark'),
    coalesce(map_theme, 'dark'),
    booking_text_color,
    ...
    blackout_ranges,
    subscription_plan,
    coalesce(booking_payment_mode, 'none'),
    trim(booking_service_area_label),
    coalesce(service_mode, 'mobile'),
    trim(shop_address),
    trim(website)
  into v_org_id, v_org_name, v_org_logo_url, v_tagline, v_map_lat, v_map_lng, v_service_radius_km,
    ...
    v_booking_text_color,
    v_booking_header_text_color,
    v_timezone, v_hours_start, v_hours_end, v_slot_interval, v_blackout_dates, v_blackout_ranges,
    v_subscription_plan, v_booking_payment_mode,
  ...
  -- Shop location: only Pro can offer shop or both
  if v_subscription_plan = 'pro' and v_service_mode in ('shop', 'both') then
    v_service_mode_public := v_service_mode;
  else
    v_service_mode_public := 'mobile';
  end if;

  -- Starter: default colours and DetailOps logo; org name (businessName) is unchanged
...
comment on function public.get_public_booking_context(text) is 'Returns public booking context. Starter: default colours and DetailOps logo. Shop location (shop/both) is Pro only.';
