...
const categories: { label: string; href: string; icon: typeof User; superAdminOnly?: boolean; locationManagerOnly?: boolean }[] = [
  { label: 'Profile', href: crmPath('/settings/profile'), icon: User, locationManagerOnly: false },
  ...
  { label: 'Schedule', href: crmPath('/settings/bookings'), icon: Calendar, locationManagerOnly: true },
  { label: 'Locations', href: crmPath('/settings/locations'), icon: MapPin },
  { label: 'Checklist', href: crmPath('/settings/checklist'), icon: ListChecks },
  { label: 'Automations', href: crmPath('/automations'), icon: Zap },
...
interface SettingsNavProps {
  ...
  /** When true (location manager), only show Profile and Schedule (location calendar). */
  locationManager?: boolean
}

export function SettingsNav({ isSuperAdmin = false, locationManager = false }: SettingsNavProps) {
  const pathname = usePathname()
  let visibleCategories = categories.filter((item) => !item.superAdminOnly || isSuperAdmin)
  if (locationManager) {
    visibleCategories = visibleCategories.filter(
      (item) => item.label === 'Profile' || item.label === 'Schedule'
    )
  }
