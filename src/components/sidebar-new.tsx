              ...
              {group.items.map((item) => {
                ...
                if (item.comingSoon) {
                  return (
                    <div
                      key={item.label}
                      className="sidebar-nav-link opacity-70"
                      style={{ cursor: 'default', pointerEvents: 'none' }}
                    >
                      <Icon className="sidebar-nav-icon h-5 w-5 shrink-0 text-[var(--text-2)]" />
                      <span style={{ flex: 1 }}>{item.label}</span>
                      <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Coming soon</span>
                    </div>
                  )
                }
                return (
