...
interface MainLayoutClientProps {
  ...
  /** Style preset: midnight, carbon, frost. Applied as base; crmColors overlay when Pro. */
  crmStylePreset?: CrmStylePresetId | null
...
export function MainLayoutClient({
  ...
  const style = useMemo(() => {
    const preset = getCrmStylePreset(crmStylePreset)
    const s: Record<string, string> = { ...preset.vars }
    ...
    const customText = crmColors?.text?.trim()
    if (customText) {
      s['--text'] = customText
      s['--text-1'] = customText
      s['--text-2'] = customText
      s['--text-3'] = customText
      s['--text-secondary'] = customText
      s['--text-muted'] = customText
    }
