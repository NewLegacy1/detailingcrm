...
export async function createConnectAccountAndLink(
  ...
  if (!stripeSecretKey) {
    return {
      error:
        'Stripe is not configured. Add STRIPE_SECRET_KEY to your .env (get it from Stripe Dashboard → Developers → API keys).',
    }
  }
  try {
...
export async function createLoginLink(accountId: string): Promise<{ url: string } | { error: string }> {
  if (!stripeSecretKey) {
    return { error: 'Stripe is not configured. Add STRIPE_SECRET_KEY to your .env.' }
  }
  try {
    const Stripe = (await import('stripe')).default
    const stripe = new Stripe(stripeSecretKey, { apiVersion: '2026-01-28.clover' })
...
/**
 * Create an AccountLink for an existing Connect account (e.g. to re-open onboarding or update details).
 * Use this when the org already has stripe_account_id so we don't create a duplicate account.
 */
export async function createAccountLinkForExisting(
  ...
  try {
    ...
    console.error('Stripe AccountLink error:', err)
