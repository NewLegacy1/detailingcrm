...
async function getValidAccessToken(
  ...
  onTokensRefreshed?: (newEncrypted: string) => void | Promise<void>
): Promise<string> {
  ...
  if (tokens.expiry_date < Date.now() + 60_000) {
    ...
    const newEncrypted = encrypt(JSON.stringify(tokens))
    await onTokensRefreshed?.(newEncrypted)
  }
...
/** Optional: when tokens are refreshed (e.g. after expiry), call this to persist so the connection stays valid. */
export type OnTokensRefreshed = (newEncrypted: string) => void | Promise<void>

export async function listCalendars(encryptedTokens: string, onTokensRefreshed?: OnTokensRefreshed): Promise<CalendarListEntry[]> {
  const accessToken = await getValidAccessToken(encryptedTokens, onTokensRefreshed)
...
export async function getCalendarSummary(encryptedTokens: string, calendarId: string, onTokensRefreshed?: OnTokensRefreshed): Promise<string | null> {
  try {
    const accessToken = await getValidAccessToken(encryptedTokens, onTokensRefreshed)
...
export async function createCalendar(encryptedTokens: string, summary: string, onTokensRefreshed?: OnTokensRefreshed): Promise<{ id: string }> {
  const accessToken = await getValidAccessToken(encryptedTokens, onTokensRefreshed)
...
export async function createEvent(encryptedTokens: string, calendarId: string, job: JobForEvent, onTokensRefreshed?: OnTokensRefreshed): Promise<string> {
  const accessToken = await getValidAccessToken(encryptedTokens, onTokensRefreshed)
...
export async function updateEvent(encryptedTokens: string, calendarId: string, eventId: string, job: JobForEvent, onTokensRefreshed?: OnTokensRefreshed): Promise<void> {
  const accessToken = await getValidAccessToken(encryptedTokens, onTokensRefreshed)
  const body = buildEventBody(job, { calendarId })
...
export async function deleteEvent(encryptedTokens: string, calendarId: string, eventId: string, onTokensRefreshed?: OnTokensRefreshed): Promise<void> {
  const accessToken = await getValidAccessToken(encryptedTokens, onTokensRefreshed)
  const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(eventId)}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${accessToken}` },
