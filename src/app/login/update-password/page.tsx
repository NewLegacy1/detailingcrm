...
/** Safe redirect: must be a relative path starting with /, not an absolute URL. */
function safeRedirectNext(raw: string | null): string | null {
...
function getPasswordResetNextCookie(): string | null {
  ...
  try {
    return safeRedirectNext(decodeURIComponent(match[1].trim()))
  } catch {
    return null
  }
}
...
function UpdatePasswordContent() {
  ...
  const nextParam = safeRedirectNext(searchParams.get('next'))
  const [cookieNext, setCookieNext] = useState<string | null>(null)
  useEffect(() => {
    setCookieNext(getPasswordResetNextCookie())
  }, [])
  const redirectTarget = nextParam ?? cookieNext ?? '/crm/dashboard'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const isBookingReturn = /^\/book\/[^/]+$/.test(redirectTarget)
      const finalUrl = isBookingReturn ? `${redirectTarget}?signin=1` : redirectTarget
            ...
            <div className="flex-1 min-h-0 overflow-y-auto auth-panel-scroll flex flex-col justify-center items-center py-10 md:py-12 px-6 sm:px-12 md:px-16">
              ...
              {success ? (
                <p className="text-[#86efac] text-sm">
                  {(nextParam ?? cookieNext)?.startsWith('/book/') ? 'Password updated. Redirecting you back to booking...' : 'Password updated. Redirecting...'}
                </p>
              ) : (
    ...
    <div className={`auth-hero-bg min-h-screen flex items-center justify-center px-4 py-8 ${plusJakarta.className}`}>
      <div className="relative w-full flex items-center justify-center">
        ...
        <div className="auth-card w-[1100px] max-w-[98vw] min-h-[520px] rounded-3xl overflow-hidden grid grid-cols-1 md:grid-cols-[45%_55%] shadow-2xl border border-[rgba(255,255,255,0.07)] relative bg-[#101620]">
          ...
          <div className="flex flex-col flex-1 min-h-0 bg-[#101620] md:border-l border-[rgba(255,255,255,0.07)] flex items-center justify-center py-12">
