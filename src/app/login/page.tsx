...
import { DetailOpsNativeLogin } from '@/components/login/DetailOpsNativeLogin'
        ...
        <div className="auth-card w-[1100px] max-w-[98vw] min-h-[520px] h-[620px] max-h-[90vh] rounded-3xl overflow-hidden grid grid-cols-1 md:grid-cols-[45%_55%] shadow-2xl border border-[rgba(255,255,255,0.07)] relative bg-[#101620]">
          ...
          <div className="flex flex-col flex-1 min-h-0 bg-[#101620] md:border-l border-[rgba(255,255,255,0.07)]">
            <div className="flex-1 min-h-0 overflow-y-auto auth-panel-scroll flex flex-col justify-center items-center py-10 md:py-12 px-6 sm:px-12 md:px-16">
              <div className="flex justify-center mb-4 md:mb-5 md:hidden">
                <Image
                  ...
                  width={160}
                  height={160}
                  className="h-40 w-40 min-h-[160px] min-w-[160px] object-contain"
                />
              </div>
              <h1 className="text-[1.6rem] font-bold text-[#dce6f5] mb-1.5 tracking-tight text-center">Welcome Back</h1>
              <p className="text-[0.82rem] text-[#4a5568] mb-8">Sign in to your DetailOps account</p>
              ...
              <form onSubmit={handleSignIn} className="w-full max-w-[360px] flex flex-col gap-3">
                ...
                <p className="text-right -mt-1">
                  <Link href="/login/forgot-password" className="text-[0.8rem] text-[#00b8f5] hover:underline">
                    Forgot password?
                  </Link>
                </p>
                <button
            ...
            <div className="shrink-0 flex justify-center gap-5 py-4 px-6 text-[0.72rem] text-[#2a3548]">
              <Link href="/privacy" className="text-[#2a3548] no-underline hover:text-[#4a6080]">
                Privacy policy
              </Link>
              <Link href="/terms" className="text-[#2a3548] no-underline hover:text-[#4a6080]">
                Terms &amp; conditions
              </Link>
            </div>
          </div>
...
function LoginSwitcher() {
  ...
  if (native === null) {
    return (
      <div className="auth-hero-bg min-h-screen flex items-center justify-center" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
  ...
  if (native) {
    return (
      <Suspense
        fallback={
          <div className="min-h-screen flex items-center justify-center bg-[#080c14]" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
            <div className="text-[#64748b]">Loading...</div>
          </div>
        }
      >
