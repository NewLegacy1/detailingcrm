...
function ForgotPasswordContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Include next in callback URL so the email link carries return path (cookie is fallback for same browser)
      const callbackPath = next && next.startsWith('/') && !next.startsWith('//')
        ? `/auth/callback?next=${encodeURIComponent(next)}`
        : '/auth/callback'
      const redirectTo = baseUrl ? `${baseUrl}${callbackPath}` : undefined
      if (typeof document !== 'undefined' && next) {
        document.cookie = `password_reset_next=${encodeURIComponent(next)}; path=/; max-age=3600; samesite=lax`
      }
        ...
        <div className="auth-card w-[1100px] max-w-[98vw] min-h-[520px] rounded-3xl overflow-hidden grid grid-cols-1 md:grid-cols-[45%_55%] shadow-2xl border border-[rgba(255,255,255,0.07)] relative bg-[#101620]">
          ...
          <div className="flex flex-col flex-1 min-h-0 bg-[#101620] md:border-l border-[rgba(255,255,255,0.07)]">
            ...
            <div className="shrink-0 flex justify-center gap-5 py-4 px-6 text-[0.72rem] text-[#2a3548]">
              ...
              <Link href="/terms" className="text-[#2a3548] no-underline hover:text-[#4a6080]">Terms and conditions</Link>
            </div>
          </div>
        </div>
      </div>
