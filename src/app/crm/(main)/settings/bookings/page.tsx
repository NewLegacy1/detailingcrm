...
export default async function SettingsBookingsPage() {
  ...
  if (orgId) {
    ...
    if (auth?.locationId) {
      const loc = await supabase.from('locations').select('name').eq('id', auth.locationId).eq('org_id', orgId).single()
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-1">
          {isLocationManager && locationName
            ? `Connect a Google Calendar for ${locationName}. Jobs at this location will sync to the calendar you choose.`
