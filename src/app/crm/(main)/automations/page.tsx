...
export default function AutomationsPage() {
  ...
  useEffect(() => {
    if (!data?.abandoned_cart_enabled) {
      setAbandonedSessions([])
      return
    }
    setSessionsLoading(true)
    fetch('/api/booking/sessions')
      .then((r) => (r.ok ? r.json() : { sessions: [] }))
      .then((json: { sessions?: AbandonedSession[] }) => setAbandonedSessions(json.sessions ?? []))
      .finally(() => setSessionsLoading(false))
  }, [data?.abandoned_cart_enabled])
  ...
  const automations = [
    ...
    },
    {
      ...
      details: [
        { label: 'Client email', value: data?.new_booking_client_email_on !== false ? 'On' : 'Off' },
        { label: 'Client SMS', value: data?.new_booking_client_sms_on !== false ? 'On' : 'Off' },
      ],
    },
    {
  ...
  return (
    <div className="space-y-8 p-6 lg:p-8" style={{ background: 'var(--bg)' }}>
      ...
      <section className="mt-8">
        ...
        <AutomationsForm orgData={data} onSaved={refetch} />
      </section>
    </div>
  )
}
