...
export function UpsellsTable({ initialUpsells, orgId }: UpsellsTableProps) {
  ...
  const [form, setForm] = useState<{ name: string; price: number; category: string; icon_url: string; service_ids: string[] | null }>({ name: '', price: 0, category: 'extras', icon_url: '', service_ids: null })
  ...
  function openCreate() {
    setEditing(null)
    setForm({ name: '', price: 0, category: 'extras', icon_url: '', service_ids: null })
    setOpen(true)
  }
  ...
  function openEdit(u: Upsell) {
    ...
    setForm({
      ...
      category: u.category,
      icon_url: u.icon_url ?? '',
      service_ids: Array.isArray(u.service_ids) && u.service_ids.length > 0 ? [...u.service_ids] : null,
    })
    setOpen(true)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      service_ids: form.service_ids && form.service_ids.length > 0 ? form.service_ids : null,
    }
    if (editing) {
  ...
  async function handleIconFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/services/upload-photo', { method: 'POST', body: formData })
      const data = await res.json()
      if (res.ok && Array.isArray(data.urls) && data.urls[0]) {
        setForm((p) => ({ ...p, icon_url: data.urls[0] }))
      }
    } finally {
                  ...
                  <TableRow key={u.id}>
                    ...
                    <TableCell className="text-sm text-[var(--text-muted)]">
                      {!u.service_ids || u.service_ids.length === 0
                        ? 'All services'
                        : services.length
                          ? u.service_ids.map((sid) => services.find((s) => s.id === sid)?.name ?? sid).filter(Boolean).join(', ') || '—'
                          : u.service_ids.length + ' selected'}
                    </TableCell>
                    <TableCell>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button
                  ...
                  disabled={iconUploading}
                >
                  {iconUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Upload image'}
                </Button>
                <span className="text-sm text-[var(--text-muted)]">or paste URL:</span>
                <Input
                  id="upsell-icon"
              ...
              {form.icon_url && (
                <div className="mt-2 flex items-center gap-2">
                  <img src={form.icon_url} alt="" className="h-10 w-10 object-contain rounded border border-[var(--border)]" />
                  <Button type="button" variant="ghost" size="sm" onClick={() => setForm((p) => ({ ...p, icon_url: '' }))}>Clear</Button>
                </div>
              )}
            </div>
            <div>
              <Label className="block mb-2">Show for services</Label>
              <p className="text-sm text-[var(--text-muted)] mb-2">
                Leave unselected to show this add-on for all services. Or select specific services to only offer it when customers book those.
              </p>
              {services.length === 0 ? (
                <p className="text-sm text-[var(--text-muted)]">No services yet. Add services first, or leave as “all services”.</p>
              ) : (
                <div className="space-y-2 max-h-48 overflow-y-auto rounded-md border border-[var(--border)] p-3 bg-[var(--surface-1)]">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={form.service_ids === null}
                      onChange={() => setForm((p) => ({ ...p, service_ids: null }))}
                      className="rounded border-[var(--border)]"
                    />
                    <span className="text-sm">All services</span>
                  </label>
                  {services.map((s) => {
                    const selected = form.service_ids !== null && form.service_ids.includes(s.id)
                    return (
                      <label key={s.id} className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={() => {
                            setForm((p) => {
                              if (p.service_ids === null) return { ...p, service_ids: [s.id] }
                              const next = selected ? p.service_ids.filter((id) => id !== s.id) : [...p.service_ids, s.id]
                              return { ...p, service_ids: next.length === 0 ? null : next }
                            })
                          }}
                          className="rounded border-[var(--border)]"
                        />
