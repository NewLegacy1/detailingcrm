...
import { Download, ExternalLink, FileText, Loader2, Mail, Search } from 'lucide-react'
...
export function InvoicesList({ initialInvoices, clients, initialCustomerId, initialJobId, initialLineItems }: InvoicesListProps) {
  ...
  async function sendReminder(row: UnifiedRow) {
    if (!row.stripeInvoiceId || row.paid) return
    setReminderSendingId(row.id)
    try {
      const res = await fetch('/api/invoices/send-reminder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ stripe_invoice_id: row.stripeInvoiceId }),
      })
                ...
                <TableRow key={row.id} className="border-white/5 text-[var(--text-secondary)] hover:bg-white/5">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      {row.source === 'local' && !row.hostedInvoiceUrl && !row.stripeInvoiceId && (
