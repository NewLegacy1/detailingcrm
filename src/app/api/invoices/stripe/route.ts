...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only list invoices for this org's connected Stripe account (not the platform or other accounts)
    const orgId = profile?.org_id ?? null
    if (!orgId) {
      return NextResponse.json({ invoices: [], hasMore: false, nextStartingAfter: null })
    }
    ...
    // Fetch invoices from this org's connected Stripe Express account only
