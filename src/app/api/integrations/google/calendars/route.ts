...
export async function GET() {
  ...
  try {
    const calendars = await listCalendars(org.google_tokens_encrypted, onTokensRefreshed)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { id } = await createCalendar(org.google_tokens_encrypted, summary, onTokensRefreshed)
    await supabase
      .from('organizations')
