...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('drip_campaigns')
    .select('id, org_id, name, trigger_type, workflow_json, active, created_at, updated_at')
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase.from('drip_campaigns').insert(payload).select('id, org_id, name, trigger_type, workflow_json, active, created_at, updated_at').single()
