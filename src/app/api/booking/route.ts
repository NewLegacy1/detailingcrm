...
export async function POST(req: NextRequest) {
  let body: {
    ...
    addressLat?: number
    addressLng?: number
    /** When 'shop', use org's shop_address as job address and skip service radius check. */
  ...
  const { data: org, error: orgError } = await supabase
    .from('organizations')
    .select('id, map_lat, map_lng, service_radius_km, shop_address, multi_location_enabled')
    .eq('booking_slug', slug)
    .single()

  if (orgError || !org?.id) {
    console.error('[Booking] Org lookup failed:', { slug, error: orgError?.message })
    return NextResponse.json({ error: 'Invalid booking link' }, { status: 404 })
  }

  const orgId = org.id
  console.info('[Booking] slug=%s org_id=%s', slug, orgId)

  let resolvedLocationId: string | null = null
  if (locationId && org.multi_location_enabled === true) {
    const { data: loc } = await supabase
      .from('locations')
      .select('id, address, service_mode')
      .eq('id', locationId)
      .eq('org_id', org.id)
      .eq('is_active', true)
      .single()
  ...
  if (serviceLocation === 'shop') {
    ...
    if (!address) {
      const shopAddress = typeof org.shop_address === 'string' ? org.shop_address.trim() : ''
      address = shopAddress
    }
    if (!address) {
  ...
  // Service radius check: when set and mobile, require lat/lng and enforce distance
  ...
  if (jobError || !newJob?.id) {
    console.error('[Booking] Job insert failed:', { orgId, slug, error: jobError?.message })
