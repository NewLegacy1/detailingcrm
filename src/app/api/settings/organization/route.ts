...
export async function PATCH(request: NextRequest) {
  ...
  if (typeof body.tax_rate === 'number') {
    const rate = Math.max(0, Math.min(100, Number(body.tax_rate)))
    if (!Number.isNaN(rate)) upd.tax_rate = rate
  }
  ...
  if (Array.isArray(body.maintenance_upsell_excluded_service_ids)) {
    const ids = body.maintenance_upsell_excluded_service_ids.filter((id: unknown) => typeof id === 'string' && /^[0-9a-f-]{36}$/i.test(id))
    upd.maintenance_upsell_excluded_service_ids = ids
  }
  ...
  if (typeof body.booking_header_text_color === 'string' || body.booking_header_text_color === null) upd.booking_header_text_color = body.booking_header_text_color
  ...
  if (typeof body.booking_service_area_label === 'string' || body.booking_service_area_label === null) upd.booking_service_area_label = body.booking_service_area_label?.trim() || null
  ...
  if (typeof body.service_mode === 'string' && ['mobile', 'shop', 'both'].includes(body.service_mode)) upd.service_mode = body.service_mode
  if (typeof body.shop_address === 'string' || body.shop_address === null) upd.shop_address = body.shop_address?.trim() || null
  if (body.shop_address_lat !== undefined) {
    if (body.shop_address_lat === null) upd.shop_address_lat = null
    else if (typeof body.shop_address_lat === 'number' && !Number.isNaN(body.shop_address_lat)) upd.shop_address_lat = body.shop_address_lat
    else if (typeof body.shop_address_lat === 'string') { const n = parseFloat(body.shop_address_lat); if (!Number.isNaN(n)) upd.shop_address_lat = n }
  }
  if (body.shop_address_lng !== undefined) {
    if (body.shop_address_lng === null) upd.shop_address_lng = null
    else if (typeof body.shop_address_lng === 'number' && !Number.isNaN(body.shop_address_lng)) upd.shop_address_lng = body.shop_address_lng
    else if (typeof body.shop_address_lng === 'string') { const n = parseFloat(body.shop_address_lng); if (!Number.isNaN(n)) upd.shop_address_lng = n }
  }
  if (typeof body.crm_style_preset === 'string' && ['midnight', 'carbon', 'frost'].includes(body.crm_style_preset)) upd.crm_style_preset = body.crm_style_preset
  if (typeof body.crm_use_custom_colours === 'boolean') upd.crm_use_custom_colours = body.crm_use_custom_colours
  if (typeof body.multi_location_enabled === 'boolean') upd.multi_location_enabled = body.multi_location_enabled
  ...
  if (org?.subscription_plan === 'starter') {
    ...
    delete upd.booking_header_text_color
